...
function(el, x) {
  ...
  function updateLegends(activeLayer) {
    if (!civicLegend || !orgLegend) return;
  ...
  map.whenReady(function() {
    // Force initial legend state
    updateLegends('Civic Opportunity Counts');

    // Rebind click handlers after everything is drawn
    map.eachLayer(function(layer) {
      if (layer.getPopup && layer.getPopup()) {
        layer.on('click', function() {
          var popup = layer.getPopup();
          if (popup) updateSidebar(popup.getContent());
          });
        }
      });
    });

  // Switch legend when base layer changes
