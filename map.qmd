...
<div style='font-family:sans-serif; font-size:13px; max-width:300px;'>
  ...
  <button onclick=\"this.nextElementSibling.style.display = this.nextElementSibling.style.display === 'none' ? 'block' : 'none';\" style='background:none; border:none; cursor:pointer; font-size:14px;'>❓</button>
  <div style='display:none; font-size:12px; margin-top:4px;'>
    This index ranks counties from 1 (low) to 5 (high) based on normalized civic opportunity scores. Each organization is scored based on whether it provides volunteering, membership, public events, or take-action opportunities. These four binary indicators (1 if offered, 0 if not) are averaged to create a score between 0 and 1 for each organization. We then sum these scores across all organizations in a county, divide by the county's population size, and split the counties into five equal-sized groups to assign index values.
  </div><br/>
  {index_boxes}
  ...
  <div style='margin-bottom:4px;'>Volunteer organizations
    <div style='background:#eee; width:100%; height:10px; position:relative;'>
      <div style='background:#1f77b4; width:{row$volunteer_sum / max(1, row$civic_org_sum) * 100}%; height:10px;'></div>
  ...
  <div style='margin-bottom:4px;'>Membership-based organizations
    <div style='background:#eee; width:100%; height:10px; position:relative;'>
      <div style='background:#ff7f0e; width:{row$membership_sum / max(1, row$civic_org_sum) * 100}%; height:10px;'></div>
  ...
  <div style='margin-bottom:4px;'>Take Action organizations
    <div style='background:#eee; width:100%; height:10px; position:relative;'>
      <div style='background:#2ca02c; width:{row$take_action_sum / max(1, row$civic_org_sum) * 100}%; height:10px;'></div>
  ...
  <div style='margin-bottom:8px;'>Event-hosting organizations
    <div style='background:#eee; width:100%; height:10px; position:relative;'>
      <div style='background:#d62728; width:{row$events_sum / max(1, row$civic_org_sum) * 100}%; height:10px;'></div>
      <div style='position:absolute; right:0; top:0; font-size:11px;'>{row$events_sum}</div>
    </div>
  </div>

  <strong>Sociodemographic Indicators (%)</strong><br/>
...
generate_orgtype_bars <- function(df) {
  paste0(
    "<div style='font-family:sans-serif; font-size:13px; max-width:300px;'>",
    "<strong>Civic Organization Breakdown</strong><br/><small>Colored bar = county share, red line = national share</small><br/>",
    paste(
      purrr::map_chr(1:nrow(df), function(i) {
        cls <- df$class[i]
        c_pct <- round(df$county_pct[i] * 100, 1)
        n_pct <- round(df$national_pct[i] * 100, 1)
        color <- org_colors[[cls]]
        if (is.null(color)) color <- "#cccccc"
...
county_names <- map_data_civic %>%
  st_drop_geometry() %>%
  select(fips = GEOID, county = NAMELSAD)
...
popup_data <- org_type_pct %>%
  ...
  complete(county = map_data_orgtype$NAMELSAD) %>%
  mutate(orgtype_bar_html = ifelse(
    is.na(orgtype_bar_html),
    "<p><strong>No civic organization type data available for this county.</strong></p>",
    orgtype_bar_html
  ))

# Step 6: Join into map_data_orgtype
map_data_orgtype <- map_data_orgtype %>%
  left_join(popup_data, by = c("NAMELSAD" = "county"))
...
combined_map <- leaflet(options = leafletOptions(minZoom = 3, zoomControl = TRUE)) %>%
  addProviderTiles("OpenStreetMap.Mapnik", group = "Base Map") %>%
  ...
  # Civic Opportunity Layer (default visible)
  addPolygons(
    data = map_data_civic,
    fillColor = ~pal_green(civic_opp_category),
    fillOpacity = 0.7,
  ...
  hideGroup("Primary Organization Type") %>%
