  ...
  addPolygons(
    ...
    popup = ~glue(
      "<div style='font-size: 13px; line-height: 1.4; max-width: 300px;'>
       ...
       Volunteer Org #: {volunteer_sum}<br/>
       Membership Org #: {membership_sum}<br/>
       Take Action Org #: {take_action_sum}<br/>
       Events Org #: {events_sum}</p>
      ...
      map.eachLayer(function(layer) {
        if (layer.feature && layer._popup) {
          layer.on('click', function() {
            updateSidebar(layer._popup.getContent());
          });
        }
      });
    }
...
htmltools::browsable(
  htmltools::tagList(
    tags$div(
      ...
      tags$div(
        id = "popup-sidebar",
        style = "width: 30%; padding: 1em; overflow-y: auto; font-size: 14px; border-right: 1px solid #ccc;"
      ),
      tags$div(
