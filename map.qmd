...
combined_map <- leaflet(options = leafletOptions(minZoom = 3, zoomControl = TRUE)) %>%
  ...
  # Add Civic Opportunity layer (this should be the *default* visible layer)
  addPolygons(
    data = map_data_civic,
    fillColor = ~ pal_green(civic_opp_category),
    fillOpacity = 0.7,
    ...
    layerId = ~paste0(NAMELSAD, ", ", STUSPS, " (", GEOID, ")"),
    group = "Civic Opportunity Counts"
  ) %>%
  # Civic Opportunity legend (explicit colors/labels to ensure match)
  addLegend(
    position = "bottomright",
    colors = c("lightgrey", "#e5f5e0", "#a1d99b", "#74c476", "#31a354", "#006d2c"),
    labels = c("No data", "1 (Lowest)", "2", "3", "4", "5 (Highest)"),
    title = "Civic Opportunity Index",
    group = "Civic Opportunity Counts",
    className = "info legend civic-legend"
  ) %>%
  # Add Primary Org layer (initially hidden)
  addPolygons(
  ...
  hideGroup("Primary Organization Type") %>% # <-- this hides the *non-default* layer
  ...
  # Add search (use layerId for unique county identification)
  addSearchFeatures(
    targetGroups = c("Civic Opportunity Counts", "Primary Organization Type"),
    options = searchFeaturesOptions(
      zoom = 7,
      openPopup = TRUE,
      textPlaceholder = "Search county (e.g., Orange County, CA)",
...
/* Bug fix: Hide org legend by default since Civic Opportunity is the default layer */
.org-legend {
  display: none;
}
