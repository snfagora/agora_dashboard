...
::: {.callout-note collapse=true title="📘 Codebook: Organizational Type Classification"}
...
::: {.callout-warning collapse=true title="Interpreting the Primary Organization Type Map and Sidebar"}
The **Primary Organization Type** layer shows the most common type of civic opportunity organization in each county, based on percentages.

- If a county has **no civic opportunity organizations at all**, it will appear **gray** or may be excluded from the map.
- If a county has **no organizations of a specific type** (e.g., Religious), that type will not appear in the sidebar chart.
- The sidebar only displays organization types that are actually present in the county — some counties may show many types, others only a few.

👉 The values represent **percentages**, not raw counts. A county with only a few organizations may still show 100% if all belong to the same type.
:::
...
map_data_orgtype <- counties %>%
  ...
  st_simplify(dTolerance = 50, preserveTopology = TRUE) %>%
  mutate(
    primary_org_class = ifelse(is.na(primary_org_class), "No data", primary_org_class),
...
pal_primary_org <- colorFactor(
  palette = c(
    ...
    "#FB9A99", "#B3DE69", "#FF69B4", "lightgrey"
  ),
...
composition_bars <- function(df) {
  ...
  max_val <- max(c(df$membership_sum, df$volunteer_sum, df$events_sum, df$take_action_sum), na.rm = TRUE)
...
combined_map <- leaflet(options = leafletOptions(minZoom = 3, zoomControl = TRUE)) %>%
  ...
  # Add Primary Org layer FIRST (so it's underneath)
  addPolygons(
  ...
  # Add Civic Opportunity layer LAST (so it's on top and default visible)
  addPolygons(
    data = map_data_civic,
    fillColor = ~ pal_green(civic_opp_category),
    fillOpacity = 0.7,
  ...
  addLegend(
    position = "bottomright",
    colors = c("#e5f5e0", "#a1d99b", "#74c476", "#31a354", "#006d2c", "lightgrey"),
    labels = c("1 (Lowest)", "2", "3", "4", "5 (Highest)", "No data"),
    title = "Civic Opportunity Index",
    group = "Civic Opportunity Counts",
    className = "info legend civic-legend"
  ) %>%
  # Layer controls (default = Civic)
  addLayersControl(
  baseGroups = c("Civic Opportunity Counts", "Primary Organization Type"),
  options = layersControlOptions(collapsed = FALSE)
) %>%
  hideGroup("Primary Organization Type") %>%
  showGroup("Civic Opportunity Counts") %>%
  # JS to dynamically switch legends
  htmlwidgets::onRender("
    function(el, x) {
      ...
      // Ensure Civic Opportunity Counts is the default on load
      map.whenReady(function() {
        setTimeout(function() {
          // Hide org legend, show civic legend
          if (orgLegend) orgLegend.style.display = 'none';
          if (civicLegend) civicLegend.style.display = 'block';
          // Find and click the Civic Opportunity radio button to activate that layer
          var radioInputs = document.querySelectorAll('.leaflet-control-layers-base input');
          radioInputs.forEach(function(input) {
            var label = input.nextSibling.textContent.trim();
            if (label === 'Civic Opportunity Counts' && !input.checked) {
              input.click();
            }
          });
        }, 100);
  ...
  ") %>%
  # JS to redirect popup to sidebar
  htmlwidgets::onRender("
    function(el, x) {
      var map = this;
