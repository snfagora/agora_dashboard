...
map_data_orgtype <- map_data_orgtype %>%
  mutate(
    primary_org_type = fct_explicit_na(primary_org_type, na_level = "NA")
  )
...
map_data_orgtype <- map_data_orgtype %>%
  mutate(primary_org_type = factor(primary_org_type, levels = org_levels))
...
map1 <- leaflet(map_data_civic, options = leafletOptions(minZoom = 3, zoomControl = TRUE)) %>%
  ...
  addPolygons(
    ...
    popup = ~glue(
      ...
      Civic Opportunity Orgs: {civic_org_sum}<br/>
      Membership Orgs: {membership_sum}<br/>
      Event Orgs: {events_sum}<br/>
      Volunteering Orgs: {volunteer_sum}<br/>
      Take Action Orgs: {take_action_sum}"
  ),
  label = ~NAMELSAD) %>%
